...
interface IJob extends Document {
  ...
  job_poster: string;
  application_list: string[]; // References to Application model
  job_deadline: string;
  job_duration: string;
...
const jobSchema = new Schema<IJob>({
  ...
  org_details: { type: Schema.Types.ObjectId, ref: "Organisation"}, // Reference to Company model
  org_name: { type: String},
  job_description: { type: String},
  job_categories: { type: [String]},
  maintainer_phono: { type: Number},
  job_poster: { type: String},
  application_list: {type: [String]}, // Reference to Application model
